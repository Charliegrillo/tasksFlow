      ...
      <div onClick={e => e.stopPropagation()} className="w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-xl max-h-[80vh] flex flex-col">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            <span className={`size-3 shrink-0 rounded-full ${milestone.color}`} />
            <div>
              ...
              <div className="flex flex-col gap-1.5">
                {bTasks.map(t => (
                  <div key={t.id} className="flex items-center gap-3 rounded-lg border border-border px-3 py-2">
                    <span className={`size-2 shrink-0 rounded-full ${statusColors[t.status] ?? 'bg-slate-400'}`} />
                    <span className="flex-1 truncate text-sm">{t.title}</span>
                    <span className="shrink-0 rounded-full bg-secondary px-2 py-0.5 text-[10px] font-medium">{statusLabels[t.status] ?? t.status}</span>
        ...
        <div className="mt-4 flex items-center justify-between gap-2 border-t border-border pt-4">
          <button onClick={exportCSV} disabled={total === 0} className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">Exportar a CSV</button>
          <button onClick={onClose} className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-muted-foreground hover:bg-secondary">Cerrar</button>          
        </div>
      </div>
    </div>
  )
