      ...
      <div onClick={e => e.stopPropagation()} className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input id="space-name" value={name} onChange={e => setName(e.target.value)} className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" placeholder="Nombre del espacio" autoFocus />
          </div>
          <div className="flex flex-col gap-1.5">
            <label htmlFor="space-password" className="text-sm font-medium">Contraseña del espacio</label>
            <input id="space-password" type="password" value={secretPassword} onChange={e => setSecretPassword(e.target.value)} className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" placeholder="Opcional: deja vacío si no quieres protegerlo" />
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-2">
              {colorOptions.map(opt => (
                <button key={opt.value} type="button" onClick={() => setColor(opt.value)} className={`size-8 rounded-full ${opt.value} transition-all ${color === opt.value ? 'ring-2 ring-offset-2 ring-offset-card ring-primary scale-110' : 'hover:scale-105'}`} aria-label={opt.label} />
              ))}
            </div>
          </div>
