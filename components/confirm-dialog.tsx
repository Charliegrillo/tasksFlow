    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-background/70 p-4" role="presentation" onClick={onCancel}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-sm rounded-sm border border-border bg-card p-6 shadow-xl">
        <div className="flex items-start gap-4">
          <div className="grid size-10 shrink-0 place-items-center rounded-sm bg-destructive/10"><AlertTriangle className="size-5 text-destructive" /></div>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button onClick={onConfirm} className="rounded-sm bg-destructive px-4 py-2 text-sm font-medium text-white hover:bg-destructive/90">{confirmLabel}</button>
        </div>
      </div>
    </div>
  )
