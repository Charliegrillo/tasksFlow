...
export function ContactSelectDialog({ open, onClose, onSelect, contacts, assignedContactIds = [] }: ContactSelectDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-background/70 p-4" role="presentation" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="flex max-h-[70vh] w-full max-w-md flex-col rounded-sm border border-border bg-card shadow-xl">
        ...
        <div className="border-b border-border px-6 py-3">
          <div className="flex items-center gap-2 rounded-sm border border-border bg-background px-3 py-2"><Search className="size-4 text-muted-foreground" /><input value={search} onChange={e => setSearch(e.target.value)} className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground" placeholder="Buscar por nombre, email o empresa..." autoFocus /></div>
        </div>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="flex flex-col gap-0.5 p-2">
              {filtered.map(contact => (
                <button key={contact.id} onClick={() => { onSelect(contact.id); onClose() }} className="flex items-center gap-3 rounded-sm px-3 py-2.5 text-left hover:bg-secondary">
