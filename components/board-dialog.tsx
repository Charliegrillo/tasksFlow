      ...
      <div onClick={e => e.stopPropagation()} className="w-full max-w-md rounded-sm border border-border bg-card p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input id="board-name" value={name} onChange={e => setName(e.target.value)} className="rounded-sm border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" placeholder="Ej: Versión 1.0" autoFocus />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium">Tipo</label>
            <div className="flex gap-2">
              {typeOptions.map(opt => (
                <button key={opt.value} type="button" onClick={() => setType(opt.value)} className={`flex-1 rounded-sm border px-3 py-2.5 text-left transition-all ${type === opt.value ? 'border-primary bg-primary/5' : 'border-border hover:border-border/80 hover:bg-secondary/50'}`}>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex gap-2">
                {paymentOptions.map(opt => (
                  <button key={opt.value} type="button" onClick={() => setPaymentStatus(opt.value)} className={`flex-1 rounded-sm border px-3 py-2 text-sm font-medium transition-all ${paymentStatus === opt.value ? opt.color + ' bg-current/5' : 'border-border text-muted-foreground hover:bg-secondary'}`}>
                    {opt.label}
                  </button>
                ))}
              </div>
