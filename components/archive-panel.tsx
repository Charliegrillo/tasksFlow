...
export function ArchivePanel({ boards, onClose, onUpdate }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-background/70 p-4" role="presentation" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-lg rounded-2xl border border-border bg-card p-6 shadow-xl max-h-[80vh] flex flex-col">
        ...
        <div className="mt-4 flex gap-2">
          {(['all', 'pagado', 'cancelado'] as const).map(s => (
            <button key={s} onClick={() => setFilter(s)} className={`rounded-lg px-3 py-1.5 text-xs font-medium ${filter === s ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:bg-secondary/80'}`}>
        ...
        <div className="mt-4 flex-1 overflow-y-auto">
          {filtered.map(board => {
            ...
            const Icon = cfg.icon
            return (
              <div key={board.id} className="flex items-center gap-3 rounded-lg border border-border px-3 py-2.5 mb-1.5">
                <div className={`grid size-8 place-items-center rounded-lg ${cfg.bg}`}><Icon className={`size-4 ${cfg.color}`} /></div>
                ...
                <div className="flex gap-1">
                  {board.paymentStatus !== 'pendiente' && (
                    <button onClick={() => onUpdate(board.id, 'pendiente')} className="rounded-lg border border-border px-2 py-1 text-[10px] font-medium text-muted-foreground hover:bg-secondary" title="Restaurar a pendiente">
                      Restaurar
                    </button>
                  )}
                  {board.paymentStatus === 'pendiente' && (
                    <>
                      <button onClick={() => onUpdate(board.id, 'pagado')} className="rounded-lg border border-border px-2 py-1 text-[10px] font-medium text-emerald-500 hover:bg-emerald-500/10">Pagado</button>
                      <button onClick={() => onUpdate(board.id, 'cancelado')} className="rounded-lg border border-border px-2 py-1 text-[10px] font-medium text-destructive hover:bg-destructive/10">Cancelado</button>
                    </>
                  )}
                </div>
              </div>
