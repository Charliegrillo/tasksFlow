...
export function CrmInteractionDialog({ open, onClose, onSave, title }: CrmInteractionDialogProps) {
  ...
  const inputClass = 'w-full rounded-sm border border-border bg-background px-3 py-2.5 text-sm text-foreground outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/10'
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-background/70 p-4 backdrop-blur-sm" role="presentation" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-lg rounded-sm border border-border bg-card p-5 shadow-2xl shadow-black/10">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">CRM</p>
            <h2 className="mt-1 text-xl font-semibold tracking-tight">{title}</h2>
          </div>
          <button onClick={onClose} className="rounded-sm p-2 text-muted-foreground transition hover:bg-secondary hover:text-foreground" aria-label="Cerrar"><X className="size-4" /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-5">
              ...
              {interactionTypes.map(it => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-sm border p-3 text-left transition ${active ? 'border-primary bg-primary/5 text-foreground shadow-sm' : 'border-border bg-background text-muted-foreground hover:border-primary/30 hover:bg-background'}`}
                  >
                    <div className="flex items-center gap-2">
                      <span className={`grid size-8 place-items-center rounded-sm ${active ? 'bg-primary text-primary-foreground' : 'bg-secondary text-foreground'}`}>
          ...
          <div className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
            ...
            <div className="rounded-sm border border-dashed border-border bg-background px-3 py-2 text-[11px] text-muted-foreground">
              {selectedType.label}
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={onClose} className="rounded-sm border border-border px-4 py-2 text-sm font-medium text-muted-foreground transition hover:bg-secondary hover:text-foreground">Cancelar</button>
            <button type="submit" disabled={!isValid} className="rounded-sm bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50">Guardar interacción</button>
          </div>
        </form>
      </div>
    </div>
