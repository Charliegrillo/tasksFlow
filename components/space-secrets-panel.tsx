...
export function SpaceSecretsPanel({ spaceId, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-background/70 p-4" role="presentation" onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="w-full max-w-2xl rounded-sm border border-border bg-card p-4 shadow-xl sm:p-6">
        ...
        {!isUnlocked ? (
          ...
          <div className="mt-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowForm(v => !v)} className="inline-flex items-center gap-2 rounded-sm bg-primary px-3 py-2 text-xs font-medium text-primary-foreground hover:bg-primary/90">
            ...
            {showForm && (
              <div className="rounded-sm border border-border bg-background/60 p-3">
                <div className="grid gap-3 sm:grid-cols-2">
                  <input value={form.name} onChange={e => setForm(v => ({ ...v, name: e.target.value }))} placeholder="Nombre del secreto" className="rounded-sm border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
                  <select value={form.type} onChange={e => setForm(v => ({ ...v, type: e.target.value as SecretType }))} className="rounded-sm border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary">
                    {typeOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}
                  </select>
                  <input value={form.value} onChange={e => setForm(v => ({ ...v, value: e.target.value }))} placeholder="Valor / contraseña / key" className="sm:col-span-2 rounded-sm border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
                  <textarea value={form.notes} onChange={e => setForm(v => ({ ...v, notes: e.target.value }))} rows={2} placeholder="Notas adicionales" className="sm:col-span-2 rounded-sm border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
                </div>
                <div className="mt-3 flex justify-end gap-2">
                  <button onClick={() => setShowForm(false)} className="rounded-sm border border-border px-3 py-1.5 text-xs font-medium text-muted-foreground hover:bg-secondary">Cancelar</button>
                  <button onClick={() => void addSecret()} disabled={!form.name.trim() || !form.value.trim()} className="rounded-sm bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">Guardar</button>
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
              {secrets.length ? secrets.map(secret => (
                <div key={secret.id} className="rounded-sm border border-border bg-background/60 p-3">
