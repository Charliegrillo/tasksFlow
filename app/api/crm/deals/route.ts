...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ data: await listCrmDeals(stageId ? Number(stageId) : undefined) })
}
...
export async function POST(req: Request) {
  ...
  return NextResponse.json({ data: await createCrmDeal(body.contactId, body.stageId, body.budgetAmount ?? 0) }, { status: 201 })
}
