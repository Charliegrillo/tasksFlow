...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ data: listCrmInteractions(Number(dealId)) })
}
...
export async function POST(req: Request) {
  ...
  return NextResponse.json({ data: createCrmInteraction(body.dealId, body.type, body.description ?? '', body.date ?? '') }, { status: 201 })
}
