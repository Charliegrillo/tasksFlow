...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ data: listSpaces(clientId || undefined) })
}
...
export async function POST(request: Request) {
  ...
  try {
    return NextResponse.json({ data: createSpace(name, body.color, Number(body.clientId) || undefined, body.secretPassword ?? null) }, { status: 201 })
  } catch {
