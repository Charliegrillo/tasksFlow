...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.archived === true) {
    const board = await archiveBoard(Number(id))
    return board ? NextResponse.json({ data: board }) : NextResponse.json({ error: 'Tablero no encontrado' }, { status: 404 })
  }
  if (body.archived === false) {
    const board = await unarchiveBoard(Number(id))
    return board ? NextResponse.json({ data: board }) : NextResponse.json({ error: 'Tablero no encontrado' }, { status: 404 })
  }
  const board = await updateBoard(Number(id), body)
  return board ? NextResponse.json({ data: board }) : NextResponse.json({ error: 'Tablero no encontrado' }, { status: 404 })
}

export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const deleted = await deleteBoard(Number(id))
