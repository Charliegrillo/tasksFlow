...
export async function GET(request: Request) {
  ...
  if (archived) return NextResponse.json({ data: await container.boardService.findBySpaceId(spaceId, true) })
  return NextResponse.json({ data: await container.boardService.findBySpaceId(spaceId) })
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json({ data: await container.boardService.create({ name, type, spaceId }) }, { status: 201 })
}
