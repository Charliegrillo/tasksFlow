...
import { createBoardList, listBoardLists } from '@/lib/db'
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ data: await listBoardLists(boardId) })
}
...
export async function POST(request: Request) {
  ...
  return NextResponse.json({ data: await createBoardList(boardId, name, body.color) }, { status: 201 })
}
